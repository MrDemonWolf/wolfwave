...
- **Fix:**
  ...
  - [ ] If keeping color, switch from the partner mashup to WolfWave's own `Brand.wolfwaveGradient` and make the stops theme-adaptive (lighten in dark mode — the navy start `#0A2540` is near-invisible on black).
...
| C. Keep partner mashup (current) | — | Shows the 3 integrations | Not the app's identity; busy; navy-free so it "works" but reads as random |
