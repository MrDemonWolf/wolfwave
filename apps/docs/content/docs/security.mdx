...
The local WebSocket server is **loopback-only** — no token or JWT is required to connect from OBS, and no auth secrets for it are stored in Keychain.
...
| `127.0.0.1:8765` | WebSocket (local) | Now-playing broadcast for OBS browser sources (loopback only) |
| `127.0.0.1:8766` | HTTP (local) | Widget HTML server (loopback only) |
