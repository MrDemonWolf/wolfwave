---
title: WolfWave Architecture — Swift, SwiftUI, EventSub
...
└── wolfwave/
    ...
    │   ├── StreamerMode.swift              # UI-only on-camera masking
    ...
    │   ├── DiagnosticsService.swift        # Opt-in MetricKit diagnostics + share card
    │   ├── Discord/
    │   │   └── DiscordRPCService.swift     # Discord Rich Presence via IPC socket
    │   ├── ListeningHistory/
    │   │   ├── ListeningHistoryService.swift # Opt-in NDJSON play log
    │   │   ├── StatsAggregator.swift       # Top artists, listening time, trends
    │   │   ├── MonthlyWrap.swift           # "Wrapped"-style monthly summary
    │   │   └── HistoryFormatting.swift     # Date/duration formatting helpers
    │   ├── Notifications/
    │   │   └── NotificationService.swift   # Opt-in song-change banner
    │   ├── SongRequest/
    │   │   ├── SongRequestService.swift    # Request flow orchestrator (chat + channel points + bits)
    │   │   ├── SongRequestQueue.swift      # Queue + hold mode + buffering + bit-cheer boost
    │   │   ├── SongRequestAccess.swift     # RequestAudience, RequestSource, presets
    │   │   ├── SkipVoteManager.swift       # Chat vote-to-skip + Twitch Polls
    ...
    │   │   ├── TwitchChannelPointsService.swift # Managed "Request a Song" reward
    ...
    │   │       ├── VoteSkipCommand.swift   # !voteskip / !vs
    │   │       └── StatsCommand.swift      # !stats (live-gated)
    │   ├── UpdateChecker/
    │   │   └── SparkleUpdaterService.swift # Sparkle framework wrapper
    │   └── WebSocket/
    │       ├── WebSocketServerService.swift # Local WS overlay broadcast (auth-token gated)
    │       └── WidgetHTTPService.swift     # Local HTTP widget server (auto-injects token)
    ├── Views/
    │   ├── SettingsView.swift              # NavigationSplitView shell
    │   ├── About/AboutSettingsView.swift
    │   ├── Advanced/AdvancedSettingsView.swift
    │   ├── AppVisibility/AppVisibilitySettingsView.swift
    │   ├── Debug/                          # DEBUG-only developer tooling tab
    │   ├── Discord/DiscordSettingsView.swift
    │   ├── HistoryStats/
    │   │   ├── HistoryStatsSettingsView.swift
    │   │   ├── StatsChartsView.swift       # SwiftUI Charts
    │   │   └── MonthlyWrapView.swift
    │   ├── MusicMonitor/MusicMonitorSettingsView.swift
    │   ├── Notifications/NotificationsSettingsView.swift
    │   ├── SongRequest/
