...
Enable Discord Rich Presence in **Settings → Discord Integration**. WolfWave connects automatically via Discord's local IPC socket — no bot token or server required.
...
<Callout type="info">
  Commands can be toggled on/off in **Settings → Twitch Integration → Bot Commands**. You can also configure global and per-user cooldowns to prevent command spam — moderators and the broadcaster bypass all cooldowns.
</Callout>
