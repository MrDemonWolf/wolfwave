...
- **Release** (`.github/workflows/release.yml`): Creates a **draft** GitHub Release on tag push (`v*`). The developer then builds, signs, and notarizes locally.
...
Code signing, notarization, and DMG creation are handled locally — the CI workflow only needs the client IDs to create a valid build for the draft release. `GITHUB_TOKEN` is provided automatically by GitHub Actions.
...
This creates a draft GitHub Release. After the workflow completes, build and notarize locally with `make prod-build && make notarize`, upload the DMG to the draft release, and publish it.
