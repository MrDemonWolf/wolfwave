...
The Twitch Client ID is stored in `Config.xcconfig`:

- This file is gitignored to prevent accidental commits
- Each developer uses their own Client ID
- The value is baked into `Info.plist` at build time

<Callout type="info">
  The Client ID is a public identifier, not a secret. It identifies your
  application to Twitch.
</Callout>
