...
- **Back up & restore settings**. Export your preferences to a plain JSON file from Settings → Advanced, then import them on another Mac or after a reinstall. Accounts and secrets are never included (the Twitch token, user/channel IDs, and the Stream Widgets auth token stay in the Keychain). On import you choose, per account, whether to reconnect Twitch; decline and that account is skipped while the rest of your preferences are merged in. The backup file lists only portable settings, so you can open and read it.
- **Appearance toggle**. Pick Light, Dark, or System in Settings → General. System follows macOS automatically (including the light/dark schedule). Light and Dark override it app-wide, menu bar menu included. The picker shows System Settings-style appearance thumbnails (#243). Settings cards now sit on an opaque macOS surface, so they no longer bloom bright in dark mode or glare near-white in light mode (#242), and the Advanced diagnostics log buttons are regrouped into a uniform 2-up grid.
- **Discord idle & pause controls**. New "When not playing" section in Settings → Discord. _Show idle status_ keeps a "Listening to WolfWave · Idle" marker on your profile when nothing is playing instead of clearing it; _Hide track while paused_ clears the profile when you pause rather than leaving the paused track up. Both off by default. The Discord preview card now mirrors real behavior, showing the live track only while playing or paused and switching to a matching empty/idle state when playback stops, Apple Music is closed, or Discord isn't running, instead of always showing a sample song.
...
- **Monthly Wrap** — a personal "wrapped"-style summary for any month. Save the card to disk as a PNG, or send it straight to Messages, Mail, AirDrop, and friends via the native macOS share sheet.
...
- **Test suite consolidation** — merged `SongCommandTests` + `LastSongCommandTests` into a single parameterised `TrackInfoCommandTests`; renamed `MusicPlaybackMonitorTests` → `AppleMusicSourceTests` to match the post-refactor class names. The suite is now more than 880 tests across 86 files.
...
- **Settings sidebar regrouped by purpose** (#239). The sidebar is grouped by what each section is for, and the App group is ordered by how often you reach for it. App Visibility is now a native single-column picker for showing WolfWave in the menu bar, the Dock, or both (#241), and the General tab lays Startup and Display Mode out side by side (#235).
...
- **One recovery card for denied Music access**. Settings → General no longer stacks the same "Music access denied" warning in three places. A single recovery card now owns the explanation and the Open System Settings action, the Sync Music card hides its duplicate permission row while access is denied, and the now-playing card reads a calm orange "Paused" instead of a red "Denied". "Try again" shows a spinner and a "still off" hint instead of feeling like a dead button (#240).
