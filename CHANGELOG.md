...
- **Song Requests.** Viewers can request songs in Twitch chat with `!sr <song>`. Requests play through Apple Music without stealing focus from your stream. Each request is added to a dedicated "WolfWave Requests" playlist in your library and played from there, so playback keeps working on macOS 26 (Tahoe), where Apple Music stopped letting apps play catalog songs that aren't saved. When the queue runs dry, Apple Music's own autoplay can keep the music going.
- **`!queue` / `!myqueue`.** Show the whole request queue, or just a viewer's own requests, in chat.
- **`!playlist` command.** Posts a link to your song-request playlist so viewers can browse what's been queued. WolfWave gathers every request into the "WolfWave Requests" playlist; make it public once in Music, tap **Fetch link** in Settings, and `!playlist` shares the live link (it stays current as new songs are added). A one-tap **Open playlist in Music** button jumps you straight to it. Off by default.
