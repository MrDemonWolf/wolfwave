...
Bare `Slider` controls have no numeric indicator next to the thumb — the user has to guess "what does the current value mean?". Used by the Twitch cooldown rows so the user can see "15s" change as they drag.
