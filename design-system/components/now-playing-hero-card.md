...
## Purpose
The "Now Playing" hero on the General tab — 92pt album art, title/artist/album, and a scrubber. Composes `AlbumArtView` for the artwork tile and `.cardStyleUnpadded()` for the card background (content-layer `controlBackgroundColor`, not glass).
...
## Tokens used
- `DSDimension.Settings.cardCornerRadius` (14) — card corner radius via `.cardStyleUnpadded()`
